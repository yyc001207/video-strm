...
| 任务 | 处理路径/输出目录/限流参数，强关联服务器（表格展示服务器地址、可按服务器筛选）；支持按目录「快速添加」批量生成任务配置，复制、单删、批量删除 |
...
3. **任务配置**：手动新建（选择所属服务器 + 填写处理路径/输出目录/限流）或使用「快速添加」：选服务器 → 父级目录 → 目录树勾选目录（逐层展开、可搜索缓存、可刷新缓存）→ 可选"输出目录带父级目录"（选中以父级目录开头，如 `/emby/电视剧`；不选以一级子目录开头，如 `/电视剧`）→ 一键生成多条任务配置（自动关联所选服务器；季文件夹自动命名为「上级名S季号」）；表格展示服务器地址，可按服务器筛选
